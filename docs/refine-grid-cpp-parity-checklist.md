...
| 2.2 | Each pass: re-warp every tile in full into mosaic space using current grid transform (`prewarp_tiles`, L1573–1596); `warp<>` uses **nearest-neighbor** (`common.hxx` L4858) | `_prewarp_tile_for_grid_refine` per tile per pass with `interpolation_order=0` | **Port** |
